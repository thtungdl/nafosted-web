...
export async function saveStore(type, incoming, editor) {
  ...
  if (type === "cong") {
    ...
    // reports = mảng báo cáo tiến độ tuần (mỗi phần tử kèm mảng phụ lục) — thay toàn bộ mỗi lần lưu,
    // giống cách giaoviec.custom đang làm, để cho phép xóa report ở client.
    if (incoming.reports) store.reports = incoming.reports;
  }
