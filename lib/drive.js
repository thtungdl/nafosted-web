...
const TYPE_DEFAULT = {
  ...
  tonghop:   { items: [] },     // tổng hợp báo cáo tuần (AI định kỳ) — mỗi item = 1 tuần (nam/thang/tuanSo + tiến độ/vấn đề/định hướng)
};
...
export async function saveStore(type, incoming, editor) {
  ...
  if (type === "cong") {
    ...
    // items = mảng bản tổng hợp tuần (mỗi phần tử = 1 tuần) — thay toàn bộ mỗi lần lưu.
    if (incoming.items) store.items = incoming.items;
  }
